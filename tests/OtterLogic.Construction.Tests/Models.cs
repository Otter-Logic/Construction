...
/// <summary>
/// Structural models as plain arrays, built so what the sequencer should find is
/// known by construction — the same arrays a Grasshopper component would hand over.
/// </summary>
internal sealed class Models
{
    ...
    public ErectionSequenceResult Sequence(ErectionSequenceOptions? options = null)
        => ErectionSequence.Plan(Rows(_starts), Rows(_ends), _supports.Count > 0 ? Rows(_supports) : null, options);
